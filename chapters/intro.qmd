...
::: {.column-margin .callout-warning title="Review comment"}
MVP: The statement underexploited, not sure if it is entirely correct especially nowadays with everyone using satellite data. Perhaps if you wanted to say that it is underexplored for social scientists and policy makers, in a way that they would need something ready to use rather than have experience on processing images. Somehow if you could make it more specific in this first sentence would look better. I know that we have social science and policy as a title but stil I think this sentence needs to be more specific.
:::

::: {.column-margin .callout-warning title="Review comment"}
EP: I agree we should be careful about the term underexploited here. I would rephrase to something slightly softer
:::

Satellite imagery offers a powerful and underexploited data solution. For more than half a century, satellites have orbited the Earth, capturing a detailed record of its surface and atmosphere. Initially designed for environmental monitoring, these data now represent an unparalleled observational archive of human and natural systems. Imagery captures the physical fabric of places, density of buildings, presence of green and blue spaces, road networks and informal settlements. ::: {.column-margin .callout-warning title="Review comment"} MVP: Should you also say here and many other features of the Earth's surface? Satellite data represent more than what the sentence says. ::: It does this consistently across the globe and can capture change at multiple scales from local neighbourhood areas to national borders or entire continents. Unlike most traditional forms of social data, which are geographically fragmented and slow to update, imagery is comprehensive, timely and replicable.

::: {.column-margin .callout-warning title="Review comment"} MVP: Should you also say here and many other features of the Earth's surface? Satellite data represent more than what the sentence says. :::
...
::: {.column-margin .callout-warning title="Review comment"}
MVP: I am thinking that from the intro we need to specify what is coarse =, high resolution and very high resolution imagery because it might be different in people's mind. We can say coarse (km) medium (10s of metres) high (metre level) very high (decimetre to cm level). The earlier we specify this the better for the reader.
:::
...
::: {.column-margin .callout-warning title="Review comment"}
MVP: At some point in the future would be nice to add a graph that shows various temporal vs spatial resolution per satellite for different applications, there are graphs like that but perhaps we could focus on social type of applications, because most graphs are on ecology or environmental etc.
:::

Satellite missions provide regular and predictable revisits ranging from daily (e.g. PlanetScope) to a few days or weeks (e.g. Sentinel-2 and Landsat). This cadence allows researchers and policymakers to generate consistent time series and monitor rapid changes in near real-time. For example, vegetation indices derived from multispectral imagery can track seasonal dynamics in urban greenness, while radar imagery can detect flood extents immediately after a storm. Such timeliness is difficult to achieve with traditional data sources, which are often updated only every few years. There is often a trade-off in satellite data between resolution and frequency, but technological advances such as nanosat constellations[^1] are constantly pushing outwards the frontier at which this trade-off takes place.

[^1]: Fleets of shoe-box sized satellites that can obtain ever higher resolution at very high frequencies.
...
::: {.column-margin .callout-warning title="Review comment"}
EP: I would suggest the above needs repositioning now that we are going with RGB - even from now, so that it addresses both green and built environments and their effects on health and well being. Concrete vs. parks story.
:::
...
::: {.column-margin .callout-warning title="Review comment"}
MVP: We need this reference for Google Earth Engine https://www.sciencedirect.com/science/article/pii/S0034425717302900
:::
...
::: {.column-margin .callout-warning title="Review comment"}
EP: I agree with Shaonlee - the gap being addressed should come higher up.
:::
...
::: {.column-margin .callout-warning title="Review comment"}
EP: I know this is a work in progress but I think eventually, perhaps not in the intro this list of applications needs to be more structured and at least a bit more complete. Feels a bit like random (although very notable) examples. Needs tobe substantiated.
:::
...
::: {.column-margin .callout-warning title="Review comment"}
MVP: I have a similar feeling that this has been rushed a bit.
:::
